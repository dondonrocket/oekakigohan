            ...
            $result = OpenAI::chat()->create([
                ...
                'messages' => [
                    [
                        'role' => 'user',
                        'content' => [
                            ["type" => "text", "text" => "あなたは絵を描くことが得意な料理研究家です。以下の画像は子どもが描いた料理です。この画像から、描かれている料理名を1つ特定してください。
                            条件
                            1.回答は必ず料理名のみ。 それ以外の説明は不要です。
                            2.色や形状から味付けや特徴を判断し、具体的な料理名にして下さい。（例：明太子パスタ、醤油ラーメン）
                            3.特定できない場合、色のみで判断し、その色を使った料理名を挙げて下さい。
                            4.スパゲッティという料理名は使用せず『パスタ』という表現を使うこと。例えば、『ペペロンチーノ』『明太子パスタ』『きのこパスタ』のようにする。
                            5.料理名を特定できない場合でも「不明」や「特定できません」とは答えず、画像の色を基に料理名を提示してください。
                            例えば：
                            - 赤系統が多い場合 → トマトパスタ、ミネストローネ、チキンのトマト煮
                            - 黄色系統が多い場合 → オムライス、カレー、スクランブルエッグ
                            - 緑系統が多い場合 → サラダ、ジェノベーゼパスタ、ほうれん草キッシュ
                            このルールに従い、必ず料理名を挙げてください。
                            出力形式
                            料理名のみ。"],
